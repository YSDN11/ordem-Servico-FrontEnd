// Serviço de ordens (utilizado por OrdersPage, EditOrderPage, CreateOrderPage)
...
export async function getOrderAndTasksById(id) {
  // usado na EditOrderPage
  const [resOrder, resTasks] = await Promise.all([
    fetch(`${JSON_SERVER_BASE}/orders/${id}`),
    fetch(`${JSON_SERVER_BASE}/tasks`)
  ]);

  if (!resOrder.ok || !resTasks.ok) {
    throw new Error('Erro ao buscar dados da ordem (json-server).');
  }
